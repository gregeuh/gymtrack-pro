...
export default function WorkoutTracker() {
  ...
  // --- MODÈLES PAR DÉFAUT (AJOUTE DU VOLUME ET DE LA LOGIQUE) ---
  const defaultTemplates: WorkoutTemplate[] = [
    {
      ...
      exercises: [
        ...
        { exerciseId: 'legext', exerciseName: 'Leg Extension', sets: [{ reps: 12, weight: 30 }, { reps: 12, weight: 30 }] }
      ],
      createdAt: new Date()
    },
    {
      ...
      exercises: [
        ...
        { exerciseId: 'inclined', exerciseName: 'Développé incliné', sets: [{ reps: 10, weight: 40 }, { reps: 10, weight: 40 }] },
        { exerciseId: 'dips', exerciseName: 'Dips', sets: [{ reps: 12 }, { reps: 12 }] }
      ],
      createdAt: new Date()
    },
    {
      id: 'default-pull', userId: 'system', name: 'Séance Tirage (Dos/Biceps)',
      exercises: [
        { exerciseId: 'pullups', exerciseName: 'Tractions', sets: [{ reps: 8 }, { reps: 8 }, { reps: 8 }] },
        { exerciseId: 'rowing', exerciseName: 'Rowing barre', sets: [{ reps: 10, weight: 40 }, { reps: 10, weight: 40 }] },
        { exerciseId: 'latpull', exerciseName: 'Tirage vertical', sets: [{ reps: 12, weight: 45 }, { reps: 12, weight: 45 }] },
        { exerciseId: 'curls', exerciseName: 'Curl biceps', sets: [{ reps: 12, weight: 12 }, { reps: 12, weight: 12 }] }
      ],
      createdAt: new Date()
    }
  ];
  ...
  // --- RENDU (DÉTAILLÉ POUR CHAQUE COMPOSANT) ---
  return (
    <div className="max-w-3xl mx-auto space-y-6 md:space-y-8 px-2 overflow-x-hidden">
      <header className="flex flex-wrap items-center justify-between gap-4">
        <div className="flex items-center gap-3">
          <button onClick={() => setCurrentPage('dashboard')} className="p-2 rounded-xl border dark:border-zinc-800 hover:bg-zinc-800 transition-colors">
        ...
        <div className="flex items-center gap-2">
          ...
          <div className={`flex items-center gap-2 p-1 px-3 rounded-2xl border ${theme === 'dark' ? 'bg-zinc-900 border-zinc-800' : 'bg-white border-zinc-200'}`}>
            ...
            <input 
              ...
              className="w-10 bg-transparent border-none text-sm font-bold focus:ring-0 p-0 text-center outline-none"
            />
      ...
      <div className="space-y-6">
        <div className="flex flex-col sm:flex-row items-start sm:items-center gap-4">
          <input type="text" placeholder="Nom de la séance (ex: Jambes Lundi)" value={workoutName} onChange={(e) => setWorkoutName(e.target.value)} className="flex-1 text-xl md:text-2xl font-bold bg-transparent border-none focus:ring-0 placeholder:text-zinc-600 dark:text-white" />
          <div className="flex gap-2">
            <button onClick={() => setShowTemplatePicker(true)} className="p-2.5 rounded-xl border dark:border-zinc-800 dark:bg-zinc-900"><Layout className="w-5 h-5" /></button>
            {selectedExercises.length > 0 && (
              <button onClick={saveAsTemplate} className="flex items-center gap-2 px-4 py-2 rounded-xl border border-dashed text-xs font-bold dark:border-zinc-800 text-zinc-500"><Copy className="w-4 h-4" /> Sauver modèle</button>
            )}
          </div>
        </div>
        ...
        <div className="space-y-6">
          {selectedExercises.map((ex, exIdx) => (
            <motion.div key={exIdx} initial={{ opacity: 0, y: 15 }} animate={{ opacity: 1, y: 0 }} className={`p-4 md:p-6 rounded-[2rem] border ${theme === 'dark' ? 'bg-zinc-900 border-zinc-800' : 'bg-white border-zinc-200 shadow-sm'}`}>
                  ...
                  <div>
                    <div className="flex items-center gap-2">
                      ...
                      {exercises.find(e => e.id === ex.exerciseId)?.videoUrl && (
                        <button onClick={() => setShowVideoModal(exercises.find(e => e.id === ex.exerciseId)!.videoUrl!)} className="p-1.5 text-red-500 hover:bg-red-500/10 rounded-xl transition-colors"><Play className="w-4 h-4 fill-current" /></button>
                      )}
                    </div>
                  </div>
                </div>
              ...
              {/* LOGIQUE DÉTAILLÉE CARDIO VS MUSCU */}
                    ...
                    <div className="min-w-[500px]">
                      ...
                      {ex.sets.map((set, setIdx) => (
                        <div key={setIdx} className="grid grid-cols-7 gap-2 items-center mb-2">
                          ...
                          <input type="number" value={set.duration || ''} onChange={(e) => updateSet(exIdx, setIdx, 'duration', parseFloat(e.target.value))} className="w-full text-center py-2.5 rounded-xl border text-xs font-bold dark:bg-zinc-800 dark:border-zinc-700 outline-none" />
                          <input type="number" value={set.distance || ''} onChange={(e) => updateSet(exIdx, setIdx, 'distance', parseFloat(e.target.value))} className="w-full text-center py-2.5 rounded-xl border text-xs font-bold dark:bg-zinc-800 dark:border-zinc-700 outline-none" />
                          <input type="number" value={set.speed || ''} onChange={(e) => updateSet(exIdx, setIdx, 'speed', parseFloat(e.target.value))} className="w-full text-center py-2.5 rounded-xl border text-xs font-bold dark:bg-zinc-800 dark:border-zinc-700 outline-none" />
                          <input type="number" value={set.resistance || ''} onChange={(e) => updateSet(exIdx, setIdx, 'resistance', parseFloat(e.target.value))} className="w-full text-center py-2.5 rounded-xl border text-xs font-bold dark:bg-zinc-800 dark:border-zinc-700 outline-none" />
                          <input type="number" value={set.cadence || ''} onChange={(e) => updateSet(exIdx, setIdx, 'cadence', parseFloat(e.target.value))} className="w-full text-center py-2.5 rounded-xl border text-xs font-bold dark:bg-zinc-800 dark:border-zinc-700 outline-none" />
                  ...
                  <>
                    ...
                    {ex.sets.map((set, setIdx) => (
                      <div key={setIdx} className="grid grid-cols-4 gap-4 items-center group">
                        ...
                        {/* CALCULATEUR DE PLAQUES INTÉGRÉ ICI */}
                        <div className="relative">
                          <input type="number" value={set.weight || ''} onChange={(e) => updateSet(exIdx, setIdx, 'weight', parseFloat(e.target.value))} className="w-full text-center py-3 rounded-2xl border font-bold dark:bg-zinc-800 dark:border-zinc-700 outline-none" />
                          <button onClick={() => setShowPlateCalculator(set.weight || 0)} className="absolute -right-2 top-1/2 -translate-y-1/2 p-2 text-blue-500 hover:scale-110 transition-all opacity-0 group-hover:opacity-100 hidden md:block"><Calculator className="w-4 h-4" /></button>
                        </div>
                        <input type="number" value={set.reps || ''} onChange={(e) => updateSet(exIdx, setIdx, 'reps', parseInt(e.target.value))} className="w-full text-center py-3 rounded-2xl border font-bold dark:bg-zinc-800 dark:border-zinc-700 outline-none" />
      ...
      {/* --- TOUTES LES FENÊTRES MODALS (COMPLÈTES) --- */}
              ...
              <div className="flex-1 overflow-y-auto p-6 space-y-2">
                {exercises.map(ex => (
                  <button key={ex.id} onClick={() => addExercise(ex)} className="w-full flex items-center justify-between p-5 rounded-3xl hover:bg-zinc-800 transition-all text-left">
                    <div><p className="font-bold text-lg">{ex.name}</p><p className="text-sm text-zinc-500 uppercase">{ex.category}</p></div>
                    <div className="p-2 bg-blue-600/10 rounded-xl"><Plus className="w-5 h-5 text-blue-600" /></div>
                  </button>
                ))}
              </div>
            </motion.div>
      ...
      <AnimatePresence>{showVideoModal && (
        <div className="fixed inset-0 z-[110] flex items-center justify-center p-4 bg-black/95 backdrop-blur-xl" onClick={() => setShowVideoModal(null)}>
          <div className="relative w-full max-w-5xl aspect-video rounded-[2.5rem] overflow-hidden shadow-2xl border border-zinc-800 bg-black">
            <iframe 
              ...
              className="w-full h-full border-none" allow="accelerometer; autoplay; encrypted-media; gyroscope; picture-in-picture" allowFullScreen 
            />
          </div>
        </div>
      )}</AnimatePresence>
