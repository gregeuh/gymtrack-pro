...
export default function WorkoutTracker() {
  ...
  const saveWorkout = async () => {
    ...
    try {
      const workoutData: Omit<Workout, 'id'> = {
        ...
        duration: duration, // --- UTILISATION DE LA DURÉE RÉELLE ---
  ...
  return (
    <div className="max-w-3xl mx-auto space-y-6 md:space-y-8 px-2 overflow-x-hidden">
      <header className="flex flex-wrap items-center justify-between gap-4">
        ...
        <div className="flex items-center gap-2">
          ...
          <button
            ...
            className="flex items-center gap-2 bg-blue-600 hover:bg-blue-700 disabled:opacity-50 text-white font-bold py-2.5 px-5 rounded-2xl transition-all shadow-lg active:scale-95 text-sm"
          >
      ...
      <div className="space-y-6">
        <div className="flex flex-col sm:flex-row items-start sm:items-center gap-4">
          <input
            ...
            className={`flex-1 text-xl md:text-2xl font-bold bg-transparent border-none focus:ring-0 placeholder:text-zinc-600 ${theme === 'dark' ? 'text-white' : 'text-zinc-900'}`}
          />
          <div className="flex gap-2">
            <button
              onClick={() => setShowTemplatePicker(true)}
              className={`p-2.5 rounded-xl border ${theme === 'dark' ? 'bg-zinc-900 border-zinc-800' : 'bg-white border-zinc-200'}`}
            >
            ...
            {selectedExercises.length > 0 && (
              <button
                ...
                className={`flex items-center gap-2 px-4 py-2 rounded-xl border border-dashed text-xs font-bold ${theme === 'dark' ? 'border-zinc-800 text-zinc-500' : 'border-zinc-200 text-zinc-400'}`}
              >
        ...
        <div className="space-y-6">
          {selectedExercises.map((ex, exIdx) => (
            <motion.div
              ...
              className={`p-4 md:p-6 rounded-[2rem] border ${theme === 'dark' ? 'bg-zinc-900 border-zinc-800' : 'bg-white border-zinc-200 shadow-sm'}`}
            >
                  ...
                  <div>
                    ...
                    {lastPerformances[ex.exerciseId] && (
                      <p className="text-[10px] text-zinc-500 font-bold uppercase tracking-wider">
                        Précédent : {lastPerformances[ex.exerciseId].sets.map(s => 
                          s.duration ? `${s.duration}m` : `${s.weight}kg x ${s.reps}`
                        ).join(' • ')}
                      </p>
                    )}
                  </div>
                      ...
                      {ex.sets.map((set, setIdx) => (
                        <div key={setIdx} className="grid grid-cols-7 gap-2 items-center mb-2">
                          ...
                          <div className="flex justify-center gap-1">
                            <button onClick={() => updateSet(exIdx, setIdx, 'isCompleted', !set.isCompleted)} className={`p-2 rounded-xl transition-all ${set.isCompleted ? 'bg-green-500 text-white' : 'bg-zinc-800/10 text-zinc-400'}`}><CheckCircle2 className="w-4 h-4" /></button>
                          </div>
                        </div>
                      ))}
                    </div>
                    ...
                    {ex.sets.map((set, setIdx) => (
                      <div key={setIdx} className="grid grid-cols-4 gap-4 items-center group">
                        ...
                        <div className="relative">
                          <input type="number" value={set.weight || ''} onChange={(e) => updateSet(exIdx, setIdx, 'weight', parseFloat(e.target.value))} className={`w-full text-center py-3 rounded-2xl border font-bold ${theme === 'dark' ? 'bg-zinc-800 border-zinc-700' : 'bg-zinc-50 border-zinc-200'}`} />
                          <button onClick={() => setShowPlateCalculator(set.weight || 0)} className="absolute -right-1 top-1/2 -translate-y-1/2 p-1.5 text-blue-500 opacity-0 group-hover:opacity-100 hidden md:block transition-opacity"><Calculator className="w-4 h-4" /></button>
                        </div>
                        ...
                        <div className="flex justify-center gap-2">
                          ...
                          <button onClick={() => removeSet(exIdx, setIdx)} className="p-3 text-zinc-400 hover:text-red-500 transition-colors"><X className="w-5 h-5" /></button>
                        </div>
                      </div>
                    ))}
                  </>
        ...
        {showExercisePicker && (
          <div className="fixed inset-0 z-[60] flex items-center justify-center p-4">
            ...
            <motion.div initial={{ opacity: 0, scale: 0.95, y: 20 }} animate={{ opacity: 1, scale: 1, y: 0 }} className={`relative w-full max-w-xl max-h-[85vh] overflow-hidden rounded-[2.5rem] border shadow-2xl flex flex-col ${theme === 'dark' ? 'bg-zinc-900 border-zinc-800' : 'bg-white border-zinc-200'}`}>
              <div className="p-8 border-b border-zinc-800 flex items-center justify-between"><h3 className="text-2xl font-bold">Choisir un exercice</h3><button onClick={() => setShowExercisePicker(false)} className="p-2 hover:bg-zinc-800 rounded-2xl"><X className="w-8 h-8" /></button></div>
              ...
              <div className="flex-1 overflow-y-auto p-6 space-y-3">
                {allTemplates.map((template) => (
                  <div key={template.id} className="group relative">
                    <button onClick={() => loadTemplate(template)} className={`w-full flex items-center justify-between p-6 rounded-3xl transition-all text-left ${theme === 'dark' ? 'hover:bg-zinc-800' : 'hover:bg-zinc-100'}`}>
                      <div className="flex items-center gap-4">
                        <div className={`p-3 rounded-2xl ${template.userId === 'system' ? 'bg-blue-600/10 text-blue-600' : 'bg-purple-600/10 text-purple-600'}`}><Layout className="w-6 h-6" /></div>
                        <div><p className="font-bold text-lg">{template.name}</p><p className="text-sm text-zinc-500 font-medium">{template.exercises.length} exercices</p></div>
                      </div>
                      <ChevronRight className="w-6 h-6 text-zinc-400" />
                    </button>
                    {template.userId !== 'system' && (
                      <button onClick={(e) => deleteTemplate(e, template.id)} className="absolute right-14 top-1/2 -translate-y-1/2 p-3 text-zinc-500 hover:text-red-500 opacity-0 group-hover:opacity-100 transition-all"><Trash2 className="w-5 h-5" /></button>
                    )}
                  </div>
                ))}
              </div>
      ...
      {/* Autres Modaux : Repos, Partage, Calculateur, Vidéo */}
      ...
      <AnimatePresence>{showVideoModal && (
        <div className="fixed inset-0 z-[110] flex items-center justify-center p-4 bg-black/95 backdrop-blur-xl" onClick={() => setShowVideoModal(null)}>
          <div className="relative w-full max-w-5xl aspect-video rounded-[2.5rem] overflow-hidden shadow-2xl border border-zinc-800 bg-black">
            ...
            <iframe 
              src={
                showVideoModal.includes('youtube.com') || showVideoModal.includes('youtu.be') 
                ? `https://www.youtube.com/embed/${showVideoModal.includes('v=') ? showVideoModal.split('v=')[1].split('&')[0] : showVideoModal.split('/').pop()}`
                : showVideoModal
              }
